...
class HTFObserver:
    ...
    def analyze_htf_run(self, run_id: str, shards_dir: str, 
        ...
        if zones_data:
            ...
            if zones_data:
                ...
                theory_b_zones = len([z for z in zones_data 
                                    if z.get('theoretical_basis') == 'Theory B'])
        ...
        feature_completeness_score = self._calculate_feature_completeness_score(avg_feature_coverage)
        overall_quality_score = (temporal_integrity_score + feature_completeness_score) / 2
        
        # Discovery density by regime
        discovery_density = self._calculate_discovery_density_by_regime(
            regime_counts, total_zones
        )
    ...
    def print_run_summary(self, summary: HTFRunSummary) -> None:
        ...
        print(f"🏺 Theory B zones: {summary.theory_b_zones} ({summary.dimensional_anchor_rate:.1%})")
        print()
