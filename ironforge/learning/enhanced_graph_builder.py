...
class EnhancedGraphBuilder:
    ...
    def _create_node_feature(self, event: dict[str, Any], session_context: dict[str, Any] = None) -> RichNodeFeature:
        ...
        if any(term in event_type for term in ['fvg_redelivery', 'fvg_formation', 'fair_value_gap', 'fpfvg']):
        ...
        if any(term in event_type for term in ['reversal', 'structure_shift', 'mss', 'change_of_character']):
            feature.set_semantic_event('reversal_flag', 1.0)
        
        # ICT Liquidity Sweep Detection
        if any(term in event_type for term in ['liquidity_sweep', 'sweep', 'liq_sweep', 'stop_hunt']):
        ...
        traditional[4] = ((price - session_open) / session_open) * 100 if session_open > 0 else 0.0  # % from open
        ...
        traditional[16] = self._calculate_zone_proximity(price, session_low, session_range, 0.40, session_context)   # 40% zone
        traditional[17] = self._calculate_zone_proximity(price, session_low, session_range, 0.618, session_context)  # Golden ratio
        traditional[18] = self._calculate_zone_proximity(price, session_low, session_range, 0.50, session_context)   # Equilibrium
        traditional[19] = self._calculate_zone_proximity(price, session_low, session_range, 0.20, session_context)   # Discount
        traditional[20] = self._calculate_zone_proximity(price, session_low, session_range, 0.80, session_context)   # Premium
        traditional[21] = self._calculate_dimensional_relationship(price, session_low, session_range, session_context)
        
        # MARKET STRUCTURE FEATURES (indices 22-27) - ICT Concepts
        traditional[22] = self._calculate_premium_discount_position(price, session_low, session_range)
    ...
    def _create_edge_feature(self, event1: dict[str, Any], event2: dict[str, Any]) -> RichEdgeFeature:
        ...
        if 'liquidity_sweep' in type1 and any(term in type2 for term in ['redelivery', 'fvg', 'reversal']):
        ...
        if any(term in type1 for term in ['premium', 'discount']) and any(term in type2 for term in ['premium', 'discount']):
    ...
    def _calculate_zone_proximity(self, price: float, session_low: float, session_range: float, zone_level: float, session_context: dict[str, Any] = None) -> float:
        ...
        if zone_level == 0.40:
            if distance < 0.01:  # Within 1% of 40% zone
                proximity = 1.0
                if theory_b_active:
                    self.logger.debug(f"Theory B 40% zone: price {price} within 1% (position {position:.3f})")
    ...
    def _calculate_dimensional_relationship(self, price: float, session_low: float, session_range: float, session_context: dict[str, Any] = None) -> float:
        ...
        theory_b_active = session_context and session_context.get('theory_b_final_range', False)
        if theory_b_active:
            self.logger.debug(f"Theory B: Price {price} positioned at {position:.3f} of FINAL range {session_range:.1f}")
        ...
        if abs(position - 0.40) < 0.02:  # Within 2% of 40% zone
            ...
            if theory_b_active:
                self.logger.info(f"Theory B 40% ZONE HIT: Price {price} at position {position:.3f} (final range)")
    ...
    def _calculate_premium_discount_position(self, price: float, session_low: float, session_range: float) -> float:
    ...
    def _estimate_liquidity_proximity(self, price: float, session_high: float, session_low: float) -> float:
    ...
    def _calculate_range_extension(self, price: float, session_high: float, session_low: float) -> float:
    ...
    def _calculate_event_relationship_strength(self, type1: str, type2: str) -> float:
        ...
        ict_structural = ['liquidity_sweep', 'fvg_redelivery', 'expansion_phase', 'fpfvg', 'premium', 'discount', 'order_block']
        ...
        if any(term in type1_lower for term in ict_structural) and any(term in type2_lower for term in ict_structural):
            return 0.7  # ICT structural events relate strongly
        elif any(term in type1_lower for term in ict_temporal) and any(term in type2_lower for term in ict_temporal):
            return 0.6  # ICT temporal events relate moderately
        elif any(term in type1_lower for term in ict_flow) and any(term in type2_lower for term in ict_flow):
            return 0.65  # ICT flow events relate well
        elif (any(term in type1_lower for term in ict_structural) and any(term in type2_lower for term in ict_temporal)) or \
             (any(term in type1_lower for term in ict_temporal) and any(term in type2_lower for term in ict_structural)):
    ...
    def _calculate_zone_relationship(self, price1: float, price2: float) -> float:
        ...
        high_estimate = max(price1, price2) + range_estimate/2
        ...
        elif (zone1 == 'discount' and zone2 == 'equilibrium') or \
             (zone1 == 'equilibrium' and zone2 == 'premium') or \
             (zone2 == 'discount' and zone1 == 'equilibrium') or \
             (zone2 == 'equilibrium' and zone1 == 'premium'):
            return 0.5
        else:
    ...
    def validate_theory_b_implementation(self, session_data: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        try:
            session_context = self._extract_session_context(session_data, session_data.get('events', []))
            events = session_data.get('events', [])
            ...
            if session_range > 0:
                ...
                validation_results['forty_percent_event_count'] = len(validation_results['forty_percent_events'])
