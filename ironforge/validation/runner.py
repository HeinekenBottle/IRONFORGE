...
def _validate_embeddings(embeddings_dir: Path) -> Dict[str, Any]:
    ...
    if attention_file.exists():
        ...
        checks["attention_data"] = {"status": "warning", "message": "No attention data (rank proxy mode)"}
    ...
    overall_status = "pass" if all(c.get("status") == "pass" for c in checks.values()) else "warning"
