...
class TestParsePrepareArgs:
    ...
    def test_parse_prepare_basic(self):
        ...
        args = _parse_args([
            ...
            "--discovery-json", "/path/to/discovery.json",
    ...
    def test_parse_prepare_with_validation(self):
        ...
        args = _parse_args([
            "prepare-motifs-input",
            "--discovery-json", "/path/to/discovery.json",
            "--validation-json", "/path/to/validation.json",
            "--out", "/path/to/output.json"
        ])
...
class TestMainPrepareMotifs:
    ...
    def test_main_prepare_with_validation(self, mock_prep_main):
        ...
        result = main([
            "prepare-motifs-input",
            "--discovery-json", "/fake/discovery.json",
            "--validation-json", "/fake/validation.json",
            "--out", "/fake/output.json"
        ])
    ...
    def test_end_to_end_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            validation_data = {
                ...
                "session_002": {
                    "confluence": [65.0, 70.0, 75.0] + [70.0] * 50
                }
            }
            ...
            result = main([
                "prepare-motifs-input",
                "--discovery-json", str(discovery_file),
                "--validation-json", str(validation_file),
                "--out", str(motifs_input_file)
            ])
            ...
            with patch('ironforge.motifs.scanner.run_cli_scan') as mock_scan:
                ...
                result = main([
                    "motifs",
                    "--input-json", str(motifs_input_file),
                    "--min-confluence", "65.0",
                    "--top-k", "3"
                ])
    ...
    def test_prepare_with_invalid_json(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(json.JSONDecodeError):  # Should raise JSON decode error directly
                main([
                    "prepare-motifs-input",
                    "--discovery-json", str(invalid_file),
                    "--out", str(output_file)
                ])
