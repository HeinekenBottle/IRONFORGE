        ...
        for session_idx, session_id in enumerate(sessions):
            ...
            for shard_idx in range(2):
                ...
                nodes_df = pd.DataFrame({
                    "node_id": range(session_idx * 200 + shard_idx * 100, 
                    ...
                    **{f"f{i}": [(session_idx + shard_idx + i * 0.1 + j * 0.01) % 1.0 
                                 for j in range(num_nodes)] for i in range(45)}
                })
                ...
                dst_nodes = [nodes_df.iloc[min(i + 1, num_nodes - 1)]["node_id"] 
                ...
                edges_df = pd.DataFrame({
                    ...
                    "dt": [60 + i * 30 for i in range(num_edges)],  # 60-second base + increments
                    **{f"e{i}": [(session_idx + shard_idx + i * 0.05 + j * 0.02) % 1.0 
                                 for j in range(num_edges)] for i in range(20)}
                })
...
def test_wave3_error_handling_integration(wave3_test_data):
    ...
    with patch('ironforge.learning.discovery_pipeline.get_ironforge_container',
    ...
    with patch.object(pipeline, 'build_temporal_graph',
                      side_effect=ValueError("Corrupted shard data")):
...
def test_wave3_performance_integration(wave3_test_data):
    ...
    with patch.object(pipeline, '_run_tgat_discovery') as mock_discovery:
        mock_discovery.return_value = [
            {"pattern_type": "test", "confidence": 0.8}
        ]
