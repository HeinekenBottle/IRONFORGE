...
def test_semantic_edge_enrichment():
    ...
    try:
        ...
        source_node = RichNodeFeature(
            ...
            daily_phase_sin=0.5,
            daily_phase_cos=0.5,
            session_position=0.4,
            time_to_close=90.0,
            weekend_proximity=0.1,
            absolute_timestamp=1723670400,
            day_of_week=3,
            month_phase=0.5,
            week_of_month=2,
            month_of_year=8,
            ...
            time_since_session_open=3600.0,
            normalized_time=0.4,
            price_delta_1m=0.1,
            price_delta_5m=0.2,
            price_delta_15m=0.3,
        ...
        target_node = RichNodeFeature(
            ...
            consolidation_flag=1.0,
            liq_sweep_flag=0.0,
            pd_array_interaction_flag=0.0,
            phase_open=0.0,
            phase_mid=0.0,
            phase_close=1.0,  # Different phase = transition
            # Basic features (same structure as source)
            time_minutes=120.0,
            daily_phase_sin=0.3,
            daily_phase_cos=0.7,
            session_position=0.8,
            time_to_close=30.0,
            weekend_proximity=0.1,
            absolute_timestamp=1723674000,
            day_of_week=3,
            month_phase=0.5,
            week_of_month=2,
            month_of_year=8,
            ...
            price_delta_5m=-0.1,
            price_delta_15m=-0.2,
            volatility_window=0.03,
            energy_state=0.4,
            contamination_coefficient=0.2,
            fisher_regime=2,
            session_character=1,
            cross_tf_confluence=0.6,
            timeframe_rank=1,
            event_type_id=3,
            timeframe_source=0,
            liquidity_type=1,
            fpfvg_gap_size=8.2,
            fpfvg_interaction_count=1,
            first_presentation_flag=0.0,
            pd_array_strength=0.5,
        ...
        semantic_event_link, event_causality, semantic_label_id = (
            builder._generate_semantic_edge_label(
        ...
        print(
            f"  semantic_event_link: {semantic_event_link} (0=none, 1=fvg_chain, 2=pd_sequence, 3=phase_transition, 4=liquidity_sweep)"
        )
        ...
        if is_phase_transition:
            print(
                "✅ SUCCESS: Phase transition correctly detected (mid→close, expansion→consolidation)"
            )
        else:
        ...
        if len(RichEdgeFeature.RELATION_TYPES) == 10:  # Should include new semantic types
            ...
            print(
                f"❌ ERROR: Expected 10 relation types, got {len(RichEdgeFeature.RELATION_TYPES)}"
            )
