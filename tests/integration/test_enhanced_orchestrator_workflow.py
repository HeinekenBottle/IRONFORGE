...
def test_enhanced_workflow():
    ...
    try:
        ...
        print(
            f"   Processing time: {time_patterns['analysis_metadata']['processing_time_ms']:.1f}ms"
        )
...
def test_session_processing_integration():
    ...
    try:
        ...
        test_graph = {
            ...
            "metadata": {"session_type": "london_overlap", "total_nodes": 1},
        }
        ...
        if "time_patterns" in metadata:
            ...
            structure_valid = (
                isinstance(tp.get("event_clusters"), list)
                and isinstance(tp.get("cross_tf_mapping"), dict)
                and isinstance(tp.get("clustering_stats"), dict)
                and isinstance(tp.get("analysis_metadata"), dict)
            )
