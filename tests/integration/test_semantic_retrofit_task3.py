...
def test_session_context_preservation():
    ...
    try:
        ...
        test_session_data = {
            ...
            "session_fpfvg": {"fpfvg_present": True, "fpfvg_formation": {"gap_size": 15.5}},
            "price_movements": [
                {
                    "timestamp": "09:30:00",
                    "price_level": 23500.0,
            ...
            "session_liquidity_events": [
                ...
                {"timestamp": "11:30:00", "event_type": "sweep", "target_level": "liquidity_pool"},
            ],
        }
        ...
        try:
            graph, session_metadata = builder.build_rich_graph(
                test_session_data, session_file_path="test_session.json"
            )
        ...
        if session_metadata["session_start_time"] == expected_start:
            ...
            print(
                f"  ❌ ERROR: Expected '{expected_start}', got '{session_metadata['session_start_time']}'"
            )
        ...
        if abs(price_range_pct - expected_range) < 0.1:
            print(
                f"  ✅ Price range correctly calculated: {price_range_pct}% (expected ~{expected_range}%)"
            )
        else:
            print(
                f"  ❌ ERROR: Price range {price_range_pct}% doesn't match expected {expected_range}%"
            )
        ...
        if semantic_counts["fvg_redelivery"] >= expected_redelivery:
            ...
            print(
                f"  ❌ ERROR: Expected ≥{expected_redelivery} redelivery events, got {semantic_counts['fvg_redelivery']}"
            )
        ...
        if semantic_counts["expansion_mentions"] >= 1:
            ...
            print(
                f"  ❌ ERROR: Expected ≥1 expansion mention, got {semantic_counts['expansion_mentions']}"
            )
