...
class StatisticalValidator:
    ...
    def __init__(self, confidence_level: float = 0.95, baseline_threshold: float = 0.87):
        ...
        logger.info(f"Statistical Validator initialized: {confidence_level*100}% confidence, {baseline_threshold*100}% baseline")
    ...
    def validate_pattern_quality(self, patterns: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Running statistical validation on {len(significant_patterns)} patterns from {session_name}")
            ...
            validation_results['quality_metrics'] = self._calculate_quality_metrics(pattern_metrics, session_metrics)
            
            # Validate baseline compliance
            baseline_results = self._validate_baseline_compliance(validation_results['quality_metrics'])
            ...
            logger.info(f"Statistical validation complete: {validation_results['validation_summary']['overall_quality']}")
            return validation_results
        ...
        for metric_name, values in metrics.items():
            ...
            try:
                ...
                metric_tests['outliers'] = {
                    ...
                    'clean_data_percentage': float((len(values) - np.sum(outliers)) / len(values) * 100)
                }
            except Exception as e:
            ...
            if metric_name in ['pattern_scores', 'confidence_values', 'archaeological_significance']:
                try:
    ...
    def _calculate_quality_metrics(self, metrics: Dict[str, np.ndarray], session_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(pattern_scores) > 0:
            ...
            quality['pattern_score_consistency'] = float(1.0 - np.std(pattern_scores) / np.mean(pattern_scores)) if np.mean(pattern_scores) > 0 else 0.0
            quality['patterns_above_baseline'] = float(np.sum(pattern_scores >= self.baseline_threshold) / len(pattern_scores))
        else:
        ...
        if len(arch_sig) > 0:
            ...
            quality['archaeological_consistency'] = float(1.0 - np.std(arch_sig) / np.mean(arch_sig)) if np.mean(arch_sig) > 0 else 0.0
        else:
        ...
        if len(attention) > 0:
            ...
            quality['attention_distribution_quality'] = float(1.0 - stats.entropy(attention / np.sum(attention)) / np.log(len(attention))) if len(attention) > 1 else 1.0
        else:
    ...
    def _validate_baseline_compliance(self, quality_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        compliance['pattern_percentage_compliant'] = patterns_above >= 0.80  # At least 80% of patterns above baseline
    ...
    def _generate_validation_summary(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'confidence_level': self.confidence_level,
            'recommendation': self._get_quality_recommendation(overall_quality, baseline_compliant, statistical_significance)
        }
        
        return summary
    
    def _get_quality_recommendation(self, quality: str, baseline_compliant: bool, statistically_significant: bool) -> str:
    ...
    def _generate_recommendations(self, validation_results: Dict[str, Any]) -> List[str]:
        ...
        if quality_metrics.get('mean_pattern_score', 0.0) < self.baseline_threshold:
            recommendations.append(f"Improve pattern scores - current mean {quality_metrics.get('mean_pattern_score', 0.0):.3f} below {self.baseline_threshold} threshold")
        
        if quality_metrics.get('pattern_score_consistency', 0.0) < 0.7:
            recommendations.append("Improve pattern consistency - high variance in pattern scores detected")
        ...
        if quality_metrics.get('mean_archaeological_significance', 0.0) < self.baseline_threshold:
            recommendations.append("Enhance archaeological significance - patterns lack sufficient archaeological value")
        
        # Outlier recommendations
        for metric_name, tests in statistical_tests.items():
            outliers = tests.get('outliers', {})
            if outliers.get('outlier_percentage', 0.0) > 20:
                recommendations.append(f"Address outliers in {metric_name} - {outliers.get('outlier_percentage', 0.0):.1f}% outliers detected")
        
        # Baseline compliance recommendations
        if not baseline_compliance.get('overall_compliant', False):
            recommendations.append("Critical: Address baseline compliance failures to meet 87% graduation threshold")
        
        if not recommendations:
            recommendations.append("No specific improvements recommended - patterns meet all quality criteria")
