...
class GlobalLatticeBuilder:
    ...
    def build_global_lattice(self, enhanced_sessions_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"✅ Global lattice complete: {len(self.global_nodes)} nodes, {len(self.global_edges)} edges, {len(self.global_hot_zones)} hot zones")
    ...
    def _extract_session_range(self, session_data: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            ...
            'range_midpoint': energy_state.get('session_high', 0) + energy_state.get('session_low', 0) / 2 if energy_state.get('session_high', 0) > 0 else 0
        }
        ...
        for cluster_key, cluster_nodes in price_clusters.items():
            if len(cluster_nodes) >= 2:  # At least 2 events to form hot zone
                ...
                hot_zone = {
                    ...
                    'sessions_involved': list(set([n.get('session_context', {}).get('session_name', 'unknown') for n in cluster_nodes])),
                    'market_periods': list(set([n.get('session_context', {}).get('market_period', 'unknown') for n in cluster_nodes]))
                }
            ...
            for h_node in higher_nodes:
                ...
                if connected_nodes:
                    cascade = {
                        ...
                        'cascade_spread': max([abs(h_price - n.get('price', 0)) for n in connected_nodes]) if connected_nodes else 0
                    }
    ...
    def _calculate_enrichment_statistics(self):
        ...
        baseline_density = len(self.global_nodes) / total_price_range if total_price_range > 0 else 0
        ...
        for hot_zone in self.global_hot_zones:
            ...
            hot_zone['statistical_significance'] = self._calculate_significance(event_count, baseline_density)
    ...
    def _generate_discovery_recommendations(self) -> List[Dict[str, Any]]:
        ...
        extreme_zones = [z for z in self.global_hot_zones if z.get('enrichment_category') == 'extreme_enrichment']
        if extreme_zones:
        ...
        strong_cascades = sorted(self.cascade_chains, key=lambda x: x.get('cascade_strength', 0), reverse=True)[:10]
        if strong_cascades:
            recommendations.append({
        ...
        if self.bridge_nodes:
            super_connectors = [b for b in self.bridge_nodes if b.get('bridge_type') == 'super_connector']
            if super_connectors:
                recommendations.append({
