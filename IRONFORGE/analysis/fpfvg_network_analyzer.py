...
class FPFVGNetworkAnalyzer:
    ...
    def analyze_fpfvg_network(self) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_results["redelivery_scoring"] = {
                ...
                "high_strength_redeliveries": len(
                    [s for s in redelivery_scores if s["strength"] > 0.7]
                ),
            }
            ...
            logger.info(
                f"FPFVG Network Analysis complete: {len(fpfvg_candidates)} candidates, {len(network_graph['edges'])} edges"
            )
            return analysis_results
    ...
    def _filter_and_limit_candidates(
        ...
        filtered_candidates = [
            candidate for score, candidate in scored_candidates[: self.max_candidates]
        ]

        logger.info(
            f"Filtered to {len(filtered_candidates)} candidates (from {len(candidates)}) for network construction"
        )
        return filtered_candidates

    def _create_fpfvg_candidate(
        self, event_data: Dict[str, Any], session_id: str, session_date: str, event_type: str
    ) -> Optional[Dict[str, Any]]:
        """Create standardized FPFVG candidate from event data"""
        try:
            # Extract basic information
            timestamp = event_data.get("timestamp", "")
            price_level = self._safe_float(
                event_data.get("price_level", event_data.get("target_price", 0))
            )
    ...
    def _map_event_subtype(self, event_data: Dict[str, Any], event_type: str) -> str:
        ...
        if event_type == "formation":
            ...
            event_data.get("redelivery_type", "unknown_redelivery")
    ...
    def _extract_magnitude(self, event_data: Dict[str, Any]) -> float:
        ...
        magnitude = event_data.get(
            "magnitude", event_data.get("gap_size", event_data.get("fill_percentage", 50.0))
        )
    ...
    def _calculate_zone_confluence(
        ...
        for zone in self.theory_b_zones:
            ...
            zone_flags[zone_key] = (
                zone_a_distance <= self.zone_tolerance and zone_b_distance <= self.zone_tolerance
            )
    ...
    def _find_convergences(
        self, edges: List[Dict[str, Any]], min_in_degree: int = 2
    ) -> List[Dict[str, Any]]:
        ...
        for edge in edges:
            if edge["pm_belt_hit"]:
                ...
                pm_touchpoints.append(
                    {"edge": edge, "target_node": target_node, "pm_belt_interaction": True}
                )

        return pm_touchpoints

    def _find_formation_redelivery_paths(
        self, edges: List[Dict[str, Any]], nodes: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _analyze_score_distribution(
        self, redelivery_scores: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "min": np.min(scores),
            "max": np.max(scores),
            "quartiles": {"q25": np.percentile(scores, 25), "q75": np.percentile(scores, 75)},
            "strength_categories": {
    ...
    def _test_zone_enrichment(
        self, candidates: List[Dict[str, Any]], redelivery_scores: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        total_zone_coverage = (
            len(self.theory_b_zones) * self.zone_tolerance * 2
    ...
    def _test_pm_belt_interaction(
        self, candidates: List[Dict[str, Any]], network_graph: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _test_reproducibility(
        ...
        for _ in range(100):  # Reduced iterations for performance
            ...
            zone_enrichment_rate = len(
                [
                    c
                    for c in sampled_candidates
                    if c["event_type"] == "redelivery" and c["zone_proximity"]["in_zone"]
                ]
            ) / max(1, len([c for c in sampled_candidates if c["event_type"] == "redelivery"]))

            pm_belt_rate = len([c for c in sampled_candidates if c["in_pm_belt"]]) / max(
                1, len(sampled_candidates)
            )
    ...
    def _analyze_redelivery_latency(
        self, network_graph: Dict[str, Any], redelivery_scores: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        if belt_latencies and non_belt_latencies:
            try:
                ...
                statistic, p_value = stats.mannwhitneyu(
                    belt_latencies, non_belt_latencies, alternative="two-sided"
                )
                ...
                latency_analysis["statistical_comparison"] = {
                    ...
                    "p_value": p_value,
                    "significant": p_value < self.alpha,
                    "belt_vs_non_belt_difference": np.median(belt_latencies)
                    - np.median(non_belt_latencies),
                }
    ...
    def _generate_summary_insights(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        insights["statistical_significance"] = {
            ...
            "latency_difference_significant": latency_test.get("statistical_comparison", {}).get(
                "significant", False
            ),
        ...
        insights["theory_b_validation"] = {
            ...
            "validation_status": (
                "STRONG"
                if zone_test.get("significant", False)
                and zone_test.get("enrichment_factor", 1.0) > 2.0
                else "MODERATE"
            ),
        }
        ...
        insights["pm_belt_evidence"] = {
            ...
            "evidence_strength": (
                "STRONG"
                if pm_test.get("significant", False) and pm_test.get("relative_risk", 1.0) > 2.0
                else "MODERATE"
            ),
        }
        ...
        insights["network_properties"] = {
            ...
            "motifs_detected": len(
                network_construction.get("network_motifs", {}).get("chains", [])
            ),
        }
    ...
    def _save_fpfvg_network_analysis(
        self,
                ...
                "edges": [
                    {
                        ...
                        "strength": next(
                            (
                                s["strength"]
                                for s in self._score_redelivery_strength(network_graph)
                                if s["edge_id"] == f"{edge['source']}_{edge['target']}"
                            ),
                            0.0,
                        ),
