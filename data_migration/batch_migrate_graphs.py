...
class BatchGraphMigrator:
    ...
    def discover_graph_files(self, input_dir: str, patterns: List[str] = None) -> List[Path]:
        ...
        for file_path in discovered_files:
            if ('graph' in file_path.name.lower() or 
                'graph' in str(file_path.parent).lower() or
                any(keyword in file_path.name.lower() for keyword in ['nodes', 'edges', 'discovery'])):
    ...
    def analyze_batch_requirements(self, graph_files: List[Path]) -> Dict:
        ...
        for i, file_path in enumerate(graph_files):
            ...
            try:
                ...
                analysis['file_size_stats']['min'] = min(analysis['file_size_stats']['min'], file_size)
                analysis['file_size_stats']['max'] = max(analysis['file_size_stats']['max'], file_size)
                ...
                if file_path.suffix.lower() == '.json':
                    ...
                    analysis['schema_distribution']['pickle_unknown'] = analysis['schema_distribution'].get('pickle_unknown', 0) + 1
        ...
        if analysis['total_files'] > 0:
            analysis['file_size_stats']['average'] = analysis['file_size_stats']['total'] / analysis['total_files']
        
        return analysis
    
    def migrate_single_file(self, input_file: Path, output_dir: Path, 
                           create_backup: bool = True) -> Tuple[bool, str, Dict]:
        """
        ...
        try:
            ...
            if create_backup:
                backup_file = input_file.with_suffix(f'.backup_{datetime.now().strftime("%H%M%S")}{input_file.suffix}')
        ...
        for i, file_path in enumerate(graph_files):
            ...
            if success:
                ...
                if result_info['migration_result']['nodes_migrated'] > 0:
                    ...
                    print(f"      Migrated: {result_info['migration_result']['nodes_migrated']} nodes")
                    print(f"      Features added: {result_info['migration_result']['features_added']}")
                else:
                    self.batch_stats['skipped_files'] += 1
                    print(f"      Skipped: Already {result_info['migration_result']['target_schema']}")
            else:
                print(f"   ❌ FAILED - {message}")
                self.batch_stats['failed_files'] += 1
                failed_migrations.append(result_info)
                self.batch_stats['errors'].append({
                    'file': file_path.name,
                    'error': message
                })
    ...
    def print_batch_report(self, successful_migrations: List, failed_migrations: List):
        ...
        print(f"✅ Successful: {self.batch_stats['migrated_files']} migrated + {self.batch_stats['skipped_files']} skipped")
        print(f"❌ Failed: {self.batch_stats['failed_files']}")
        
        success_rate = ((self.batch_stats['migrated_files'] + self.batch_stats['skipped_files']) / 
                       self.batch_stats['total_files'] * 100) if self.batch_stats['total_files'] > 0 else 0
...
Examples:
    ...
    parser.add_argument('--input', '-i', required=True,
                       help='Input directory containing 34D graph files')
    parser.add_argument('--output', '-o', required=True, 
                       help='Output directory for migrated 37D graph files')
    parser.add_argument('--no-backup', action='store_true',
                       help='Skip backup creation (faster but riskier)')
    parser.add_argument('--workers', '-w', type=int, default=4,
                       help='Number of parallel workers (default: 4)')
    parser.add_argument('--dry-run', action='store_true',
                       help='Analyze files without performing migration')
    ...
    try:
        ...
        if args.dry_run:
            ...
            success_rate = ((batch_stats['migrated_files'] + batch_stats['skipped_files']) / 
                          batch_stats['total_files'] * 100) if batch_stats['total_files'] > 0 else 0
